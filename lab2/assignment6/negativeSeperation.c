...
void seperate(int a[], int amount) {
  int marker = 0; // keeps track of where the segregated negative numbers end
  int temp;
  for(int i = 0; i < amount; i++) { // goes through entire array
    ...
    // invariant: all numbers between the marker and i are positive
    // ex:  for(int j = marker; j =< i; j++)
